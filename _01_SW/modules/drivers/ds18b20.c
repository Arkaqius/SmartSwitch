...
// Do a ROM select
//
void ICACHE_FLASH_ATTR ds_select(const uint8_t *rom)
{
	uint8_t i;
...
// Do a ROM skip
//
void ICACHE_FLASH_ATTR ds_skip()
{
    ds_write(DS1820_SKIP_ROM,0); // Skip ROM
}

void ICACHE_FLASH_ATTR ds_reset_search()
{
	int i;
...
uint8_t ICACHE_FLASH_ATTR ds_reset(void)
{
	int r;
	uint8_t retries = 125;
	GPIO_DIS_OUTPUT(DS18B20_PIN);
...
// more certain timing.
//
void ICACHE_FLASH_ATTR ds_write_bit(int v)
{
	GPIO_OUTPUT_SET(DS18B20_PIN, 0);
	if(v) {
		os_delay_us(10);
...
//
uint8_t ICACHE_FLASH_ATTR ds_crc8(const uint8_t *addr, uint8_t len)
{
	uint8_t crc = 0;
	uint8_t i;
	while (len--) {
...
//
uint16_t ICACHE_FLASH_ATTR ds_crc16(const uint16_t *data, const uint16_t  len)
{
	uint16_t  i;
	uint16_t  crc = 0;
    for ( i = 0; i < len; i++) {
